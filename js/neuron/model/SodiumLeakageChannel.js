...
AbstractLeakChannel
{
  ...
  final
  Color
  BASE_COLOR = ColorUtils.interpolateRBGA( NeuronConstants.SODIUM_COLOR, Color.YELLOW, 0.5 );
  ...
  final
  double
  NOMINAL_LEAK_LEVEL = 0.005;

  private
  static
  final
  double
  DEFAULT_PARTICLE_VELOCITY = 7000; // In nanometers per sec of sim time.
  ...
  private
  double
  previousNormalizedLeakCurrent = 0;

  //----------------------------------------------------------------------------
  // Constructor
  //----------------------------------------------------------------------------
  public
  SodiumLeakageChannel( IParticleCapture
  ...
  {
    MembraneCrossingDirection
    result = MembraneCrossingDirection.OUT_TO_IN;
...
)
  {
    ...
    double
    normalizedLeakCurrent = MathUtils.round( hodgkinHuxleyModel.get_l_current() / PEAK_NEGATIVE_CURRENT, 2 );
   ...
   */
  private
  void updateParticleCaptureRate( double
  normalizedRate
)
  {
    if ( normalizedRate <= 0.001 ) {
      ...
      double
      absoluteMinInterCaptureTime = 0.0002;
      double
      variableMinInterCaptureTime = 0.002;
      double
      captureTimeRange = 0.005;
      double
      minInterCaptureTime = absoluteMinInterCaptureTime + (1 - normalizedRate) * (variableMinInterCaptureTime);
