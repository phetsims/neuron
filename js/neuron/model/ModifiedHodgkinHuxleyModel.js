...
IHodgkinHuxleyModel
{
  ...
  final
  double
  INTERNAL_TIME_STEP = 0.005; // In milliseconds, not seconds.
  ...
  private
  double
  gk, gna, gl;  //constant leak permeabilties
  private
  double
  n, m, h;  // voltage-dependent gating paramaters
  private
  double
  dn, dm, dh;  //corresponding deltas
  private
  double
  an, bn, am, bm, ah, bh; // rate constants
  private
  double
  vk, vna, vl;  // Ek-Er, Ena - Er, Eleak - Er

  private
  double
  n4, m3h, na_current, k_current, l_current;
  ...
  final
  double
  MAX_DELAY = 0.001; // In seconds of simulation time.
  private
  DelayBuffer
  m3hDelayBuffer = new DelayBuffer( MAX_DELAY, NeuronDefaults.MIN_ACTION_POTENTIAL_CLOCK_DT );
  private
  DelayBuffer
  n4DelayBuffer = new DelayBuffer( MAX_DELAY, NeuronDefaults.MIN_ACTION_POTENTIAL_CLOCK_DT );
  ...
  { return vClampOn; }
  public
  void setVClampOn( boolean
  vClampOn
)
  { this.vClampOn = vClampOn; }

  float
  vClampValue = convertV( 0
  F
)
  ;
  ...
  {
    int
    modelIterationsToRun = (int)
    Math.floor( (dt * 1000) / INTERNAL_TIME_STEP );
