...
package
edu.colorado.phet.neuron.model;
...
import edu.
colorado.phet.common.phetcommon.model.clock.ClockAdapter;
import edu.
colorado.phet.common.phetcommon.model.clock.ClockEvent;
import edu.
colorado.phet.common.phetcommon.model.clock.ConstantDtClock;
import edu.
...
import edu.
colorado.phet.neuron.view.MembraneChannelNode;
import edu.
umd.cs.piccolo.PNode;
import edu.
umd.cs.piccolox.pswing.PSwing;
...
GatedChannel
{
  ...
  final
  double
  CHANNEL_HEIGHT = AxonMembrane.MEMBRANE_THICKNESS * 1.2; // In nanometers.
  private
  static
  final
  double
  CHANNEL_WIDTH = AxonMembrane.MEMBRANE_THICKNESS * 0.50; // In nanometers.
  ...
  final
  double
  MIN_INTER_CAPTURE_TIME = 0.00002; // In seconds of sim time.
  private
  static
  final
  double
  MAX_INTER_CAPTURE_TIME = 0.00010; // In seconds of sim time.
  ...
  final
  double
  M3H_WHEN_FULLY_OPEN = 0.25;

  // Possible values for internal state.
  private
enum
  GateState
  {IDLE, OPENING, BECOMING_INACTIVE, INACTIVATED, RESETTING}
  ;
  ...
  final
  double
  ACTIVATION_DECISION_THRESHOLD = 0.002;
  private
  static
  final
  double
  FULLY_INACTIVE_DECISION_THRESHOLD = 0.98;
  ...
  final
  double
  INACTIVE_TO_RESETTING_TIME = 0.001; // In seconds of sim time.
  private
  static
  final
  double
  RESETTING_TO_IDLE_TIME = 0.001; // In seconds of sim time.
  ...
  final
  double
  MAX_STAGGER_DELAY = NeuronDefaults.MIN_ACTION_POTENTIAL_CLOCK_DT * 5; // In seconds of sim time.
  ...
  public
  SodiumDualGatedChannel( IParticleCapture
  modelContainingParticles, IHodgkinHuxleyModel
  hodgkinHuxleyModel
)
  {
...
)
  {
    ...
    double
    normalizedConductance = calculateNormalizedConductance();
  ...
  @Override
  public
  void moveParticleThroughNeuronMembrane( Particle
  particle, double
  maxVelocity
)
  {
  ...
  private
  double
  mapOpennessToNormalizedConductance( double
  normalizedConductance
)
  {
    assert
    normalizedConductance >= 0 && normalizedConductance <= 1;
  ...
  final
  int
  INITIAL_INTERMEDIATE_COORD_WIDTH = 786;
  private
  static
  final
  int
  INITIAL_INTERMEDIATE_COORD_HEIGHT = 786;
  private
  static
  final
  Dimension
  INITIAL_INTERMEDIATE_DIMENSION = new Dimension( INITIAL_INTERMEDIATE_COORD_WIDTH,
...
)
  {
    ...
    ConstantDtClock
    clock = new ConstantDtClock( NeuronDefaults.CLOCK_FRAME_RATE, NeuronDefaults.DEFAULT_ACTION_POTENTIAL_CLOCK_DT );
    ...
    mvt = new ModelViewTransform2D(
      ...
      new Point( INITIAL_INTERMEDIATE_COORD_WIDTH / 2,
        (int)
    Math.round( INITIAL_INTERMEDIATE_COORD_HEIGHT * 0.5 )
  ),
    ...
    PNode
    nonMvtTestNode = new PhetPPath( new Rectangle2D.Double( 0, 0, 100, 100 ), Color.yellow );

    // Create the channel node.
    IParticleCapture
    particleCapture = new IParticleCapture()
    {
      public
      void requestParticleThroughChannel( ParticleType
      particleType,
        MembraneChannel
      membraneChannel, double
      maxVelocity, MembraneCrossingDirection
      direction
    )
      {
        // Do nothing.
    ...
    final
    IHodgkinHuxleyModel
    hhModel = new ModifiedHodgkinHuxleyModel();
    final
    SodiumDualGatedChannel
    sodiumDualGatedChannel = new SodiumDualGatedChannel( particleCapture, hhModel );
    sodiumDualGatedChannel.setRotationalAngle( Math.PI / 2 );
    MembraneChannelNode
    channelNode = new MembraneChannelNode( sodiumDualGatedChannel, mvt );
    ...
    JButton
    stimButton = new JButton( "Stimulate" );
    ...
    ;
    PSwing
    stimButtonPSwing = new PSwing( stimButton );
