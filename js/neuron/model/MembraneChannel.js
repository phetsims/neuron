...
MembraneChannel
{
  ...
  final
  double
  SIDE_HEIGHT_TO_CHANNEL_HEIGHT_RATIO = 1.3;
  protected
  static
  final
  Random
  ...
  final
  double
  DEFAULT_PARTICLE_VELOCITY = 40000; // In nanometers per sec of sim time.
  ...
  private
  IParticleCapture
  modelContainingParticles;

  // Member variables that control the size and position of the channel.
  private
  Point2D
  centerLocation = new Point2D.Double();
  private
  double
  rotationalAngle = 0; // In radians.
  private
  Dimension2D
  channelSize = new PDimension(); // Size of channel only, i.e. where the atoms pass through.
  private
  Dimension2D
  overallSize = new PDimension(); // Size including edges.

  // Variable that defines how open the channel is.
  private
  double
  openness = 0;  // Valid range is 0 to 1, 0 means fully closed, 1 is fully open.

  // Variable that defines how inactivated the channel is, which is distinct
  // from openness.
  private
  double
  inactivationAmt = 0;  // Valid range is 0 to 1, 0 means completely active, 1 is completely inactive.
  ...
  private
  CaptureZone
  interiorCaptureZone = new NullCaptureZone();
  private
  CaptureZone
  exteriorCaptureZone = new NullCaptureZone();
  ...
  private
  double
  minInterCaptureTime = Double.POSITIVE_INFINITY;
  private
  double
  maxInterCaptureTime = Double.POSITIVE_INFINITY;

  // Velocity for particles that move through this channel.
  private
  double
  particleVelocity = DEFAULT_PARTICLE_VELOCITY;
  ...
  channelWidth, double
  channelHeight, IParticleCapture
  modelContainingParticles
)
  {
  ...
  static
  MembraneChannel
  createMembraneChannel( MembraneChannelTypes
  channelType, IParticleCapture
  particleModel,
    IHodgkinHuxleyModel
  hodgkinHuxleyModel
)
  {
    ...
    assert
    membraneChannel != null; // Should be able to create all types of channels.
...
)
  {
    ...
    Shape
    channelShape = new Rectangle2D.Double(
    ...
    AffineTransform
    transform = AffineTransform.getRotateInstance( rotationalAngle, centerLocation.getX(), centerLocation.getY() );
    Shape
    rotatedChannelShape = transform.createTransformedShape( channelShape );
  ...
  protected
  void setInteriorCaptureZone( CaptureZone
  captureZone
)
  {
  ...
  protected
  void setExteriorCaptureZone( CaptureZone
  captureZone
)
  {
  ...
  public
  void setCenterLocation( Point2D
  newCenterLocation
)
  {
  ...
  public
  void setRotationalAngle( double
  rotationalAngle
)
  {
  ...
  protected
  void setInactivationAmt( double
  inactivationAmt
)
  {
  ...
  private
  void notifyInactivationAmtChanged()
  {
    for ( Listener listener : listeners
  )
    {
...
class
  ...
  protected
  void setMaxInterCaptureTime( double
  maxInterCaptureTime
)
  {
  ...
  protected
  void setMinInterCaptureTime( double
  minInterCaptureTime
)
  {
   ...
   */
  public
  void setState( MembraneChannelState
  state
)
  {
...
class
  MembraneChannelState
  {
    ...
    public
    MembraneChannelState( MembraneChannel
    membraneChannel
  )
    {
