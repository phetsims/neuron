...
MotionStrategy
{
  ...
  final
  double
  MIN_TIME_BETWEEN_VELOCITY_UPDATES = 0.0002;  // In seconds of sim time, not wall time.
  private
  static
  final
  double
  MAX_TIME_BETWEEN_VELOCITY_UPDATES = 0.0010;  // In seconds of sim time, not wall time.
  private
  static
  final
  double
  ...
  private
  double
  timeUntilNextVelocityChange; // In seconds of sim time.
  private
  MutableVector2D
  velocityVector = new MutableVector2D( 0, 0 );
  private
  Point2D
  currentLocation = new Point2D.Double();
  private
  Rectangle2D
  motionBounds = new Rectangle2D.Double();
   ...
   */
  public
  RandomWalkMotionStrategy( Rectangle2D
  motionBounds
)
  {
...
)
  {
    ...
    double
    radius = movableModelElement.getRadius();
   ...
   */
  private
  void changeVelocityVector()
  {
    double
    rotationAngle = RAND.nextDouble() * Math.PI * 2;
    double
    magnitude = MIN_VELOCITY + RAND.nextDouble() * ( MAX_VELOCITY - MIN_VELOCITY );
