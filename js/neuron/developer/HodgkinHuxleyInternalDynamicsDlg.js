...
package
edu.colorado.phet.neuron.developer;
...
import edu.
colorado.phet.common.phetcommon.application.PaintImmediateDialog;
import edu.
colorado.phet.common.phetcommon.model.clock.ClockAdapter;
import edu.
colorado.phet.common.phetcommon.model.clock.ClockEvent;
import edu.
colorado.phet.common.phetcommon.model.clock.IClock;
import edu.
colorado.phet.common.phetcommon.view.util.PhetFont;
import edu.
colorado.phet.neuron.model.IHodgkinHuxleyModel;
...
public
class
HodgkinHuxleyInternalDynamicsDlg
extends
PaintImmediateDialog
{
  ...
  private
  JTextField
  timeText = new JTextField( "000.0000 ms" );
  private
  JLabel
  voltageLabel = new JLabel( "Membrane Voltage: " );
  private
  JTextField
  voltageText = new JTextField( "00.0 mV" );
  private
  JLabel
  soduimChannelCurrentLabel = new JLabel( "Sodium Channel Current: " );
  private
  JTextField
  sodiumChannelCurrentValue = new JTextField( "00.0 mA" );
  private
  JLabel
  potassiumChannelCurrentLabel = new JLabel( "Potassium Channel Current: " );
  private
  JTextField
  potassiumChannelCurrentValue = new JTextField( "00.0 mA" );
  private
  JLabel
  leakChannelCurrentLabel = new JLabel( "Leak Channel: " );
  private
  JTextField
  leakChannelCurrentValue = new JTextField( "00.0 mA" );
  private
  JLabel
  n4Label = new JLabel( "n4: " );
  private
  JTextField
  n4Value = new JTextField( "0.00" );
  private
  JLabel
  m3hLabel = new JLabel( "m3h: " );
  private
  JTextField
  m3hValue = new JTextField( "0.00" );
  private
  JButton
  ...
  private
  double
  previousElapsedTime = 1000000;

  private
  ArrayList < HhDataCaptureEntry > captureData = new ArrayList < HhDataCaptureEntry > ();

  public
  HodgkinHuxleyInternalDynamicsDlg( Frame
  frame, IClock
  clock, IHodgkinHuxleyModel
  hhModel
)
  {
    super( frame );
  ...
  {
    double
    elapsedTime = hodgkinHuxleyModel.getElapsedTime();
    if ( elapsedTime != previousElapsedTime ) {
      double
      membraneVoltage = hodgkinHuxleyModel.getMembraneVoltage() * 1000;
      double
      currentNa = hodgkinHuxleyModel.get_na_current();
      double
      currentK = hodgkinHuxleyModel.get_k_current();
      double
      currentLeak = hodgkinHuxleyModel.get_l_current();
      double
      n4 = hodgkinHuxleyModel.get_n4();
      double
      m3h = hodgkinHuxleyModel.get_m3h();
  ...
  private
  void displayCapturedData()
  {

    JDialog
    dialog = new JDialog( this, true );
    JTextArea
    textDisplay = new JTextArea( captureData.size() + 1, 80 );
    ...
    JScrollPane
    scrollableTextDisplay = new JScrollPane( textDisplay );
...
class
  HhDataCaptureEntry
  {
    ...
    public
    HhDataCaptureEntry( double
    elapsedTime, double
    membraneVoltage, double
    currentNa,
      double
    currentK, double
    currentLeak, double
