...
import edu.
colorado.phet.neuron.model.AxonMembrane;
import edu.
colorado.phet.neuron.model.AxonMembrane.TravelingActionPotential;
import edu.
...
PNode
{
  ...
  final
  Color
  AXON_BODY_COLOR = new Color( 221, 216, 44 );
  private
  static
  final
  Stroke
  ...
  public
  AxonBodyNode( AxonMembrane
  axonMembraneModel, ModelViewTransform2D
  transform
)
  {
    ...
    // Add the axon body.
    Shape
    axonBodyShape = mvt.createTransformedShape( axonMembraneModel.getAxonBodyShape() );
    Rectangle2D
    axonBodyBounds = axonBodyShape.getBounds2D();
    Rectangle2D
    crossSectionBounds = mvt.createTransformedShape( axonMembraneModel.getCrossSectionEllipseShape() ).getBounds2D();
    Point2D
    gradientOrigin = new Point2D.Double( axonBodyBounds.getMaxX(), axonBodyBounds.getMaxY() );
    Point2D
    gradientExtent = new Point2D.Double( crossSectionBounds.getCenterX(), crossSectionBounds.getY() );
    GradientPaint
    axonBodyGradient = new GradientPaint(
      gradientOrigin,
   ...
   */
  private
  void addTravelingActionPotentialNode( TravelingActionPotential
   ...
   */
  private
  void removeTravelingActionPotentialNode()
  {
...
extends
  PNode
  {
    ...
    static
    Color
    BACKGROUND_COLOR = new Color( 204, 102, 255 );
    private
    static
    Stroke
    backgroundStroke = new BasicStroke( 20, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND );
    private
    static
    Color
    ...
    static
    Stroke
    foregroundStroke = new BasicStroke( 10, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND );

    private
    AxonMembrane.TravelingActionPotential
    travelingActionPotential;
    private
    ModelViewTransform2D
    mvt;
    private
    PhetPPath
    background = new PhetPPath( backgroundStroke, BACKGROUND_COLOR );
    private
    PhetPPath
    foreground = new PhetPPath( foregroundStroke, FOREGROUND_COLOR );

    public
    TravelingActionPotentialNode( AxonMembrane.TravelingActionPotential
    travelingActionPotential, ModelViewTransform2D
    mvt
  )
    {
