...
define( function( require ) {
  ...
  function MembraneChannelNode( membraneChannelModel, mvt ) {
    ...
    // The channel canvas node needs the edge bounds value so have edge Node as instances
    thisNode.leftEdgeNode = createEdgeNode( transformedEdgeNodeSize, membraneChannelModel.getEdgeColor() );
    thisNode.rightEdgeNode = createEdgeNode( transformedEdgeNodeSize, membraneChannelModel.getEdgeColor() );
    ...
    thisNode.edgeLayer.addChild( thisNode.leftEdgeNode );
    thisNode.edgeLayer.addChild( thisNode.rightEdgeNode );
    ...
    function updateRepresentation() {
      ...
      var edgeNodeBounds = thisNode.leftEdgeNode.getBounds();
      thisNode.leftEdgeNode.x = -transformedChannelSize.width / 2 - edgeNodeBounds.width / 2;
      thisNode.leftEdgeNode.y = 0;
      thisNode.rightEdgeNode.x = transformedChannelSize.width / 2 + edgeNodeBounds.width / 2;
      thisNode.rightEdgeNode.y = 0;
  ...
  return inherit( Node, MembraneChannelNode, {
     ...
     * THE CODE NOW USES CANVAS TO RENDER THE CHANNEL AND EDGES. (Ashraf)
     * @param channelLayer
     * @param edgeLayer
     *
     * */
    addToCanvas: function( edgeLayer ) {
      edgeLayer.addChild( this.edgeLayer );//Membrane channel maintains its own layer of 2 edge nodes
    },
    removeFromCanvas: function( edgeLayer ) {
